...
test('Robustness to legacy arguments', async ({ page }) => {
  ...
  // Manually trigger a simulation call with an extra unexpected argument via Pyodide
  const result = await page.evaluate(async () => {
    ...
    const params = {
      ...
      end_age: 70,
      spouse_death_age: 85,
      initial_roth_balance: 100000,
      initial_trad_balance: 100000,
      growth_rate: 0.05,
      pension_income: 0,
      ...
      // LEGACY ARGUMENT that caused the reported error
